...
inline osg::Node* readNodeFile(const std::u8string& filename)
{
    auto s = std::string(reinterpret_cast<const char*>(filename.c_str()), filename.size());
    return readNodeFile(s, Registry::instance()->getOptions());
}
#endif
