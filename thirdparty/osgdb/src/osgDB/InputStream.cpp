...
{
    if (getOptions() && getOptions()->getPluginStringData("godotDisableExternalResources") == "1")
    ...
    *this >> PROPERTY("WriteHint") >> writeHint >> decision;
    ...
    case IMAGE_INLINE_DATA:
        if ( isBinary() )
        {
            ...
            *this >> PROPERTY("Data");
            const unsigned int encodedLevelCount = readSize();
            *this >> BEGIN_BRACKET;

            // Some producers emit an inline-data image with no dimensions or
            // payload when only an external filename is available. Do not
            // consume the closing bracket as a Base64 string, otherwise the
            // remainder of the ASCII object stream becomes misaligned. The
            // filename is turned into a URI-only placeholder below.
            if ( encodedLevelCount==0 || s<=0 || t<=0 || r<=0 )
            {
                *this >> END_BRACKET;
                readFromExternal = false;
                break;
            ...
            unsigned int levelSize = encodedLevelCount-1;
    ...
    case IMAGE_INLINE_FILE:
        if ( isBinary() )
        {
            ...
            if ( size>0 )
            {
                ...
                if ( reader )
                {
                    osg::ref_ptr<Options> opt = new Options(ext);
                    osgDB::ReaderWriter::ReadResult rr = reader->readImage(data, size, opt.get());
                    if (rr.validImage())
                        image = rr.takeImage();
                    else
                    {
    ...
    // Godot owns all external-resource fetching. Preserve the URI in a
    // placeholder Image so the converter can return it without asking OSG to
    // touch the file system or network.
    if (!readFromExternal && !name.empty() && !image)
    {
        image = new osg::Image;
